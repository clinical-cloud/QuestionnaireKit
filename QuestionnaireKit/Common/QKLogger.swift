...
Logging levels
*/
public enum QKLogLevel: Int, CustomStringConvertible {
...
extension QKLogLevel: Comparable {
	
	static public func <(lh: QKLogLevel, rh: QKLogLevel) -> Bool {
...
The `QKDebugLogger` is a simple implementation that logs to stdout. If you need more sophisticated logging, just adapt this protocol
and set your logger on the `OAuth2` instance you're using.
*/
public protocol QKLogger {
	
	/// The logger's logging level.
	var level: QKLogLevel { get }
	
	/** This is made a protocol method so it can be implemented in `extension QKLogger` **and** you're able to create your own class
	that overrides this method so you can write your own implementation. */
	func log(_ atLevel: QKLogLevel, module: String?, filename: String?, line: Int?, function: String?, msg: @autoclosure() -> String)
...
extension QKLogger {
	...
	*/
	public func log(_ atLevel: QKLogLevel, module: String?, filename: String?, line: Int?, function: String?, msg: @autoclosure() -> String) {
...
*/
open class QKDebugLogger: QKLogger {
	
	/// The logger's logging level, set to `Debug` by default.
	open var level = QKLogLevel.debug
	
	
	/** Designated initializer. */
	public init(_ level: QKLogLevel = QKLogLevel.debug) {
		self.level = level
	}
}
